...
function cargarPagina(pagina){
	...
	if (pagina==1)
	{
		llamada_prototype('paginas/tablagoleadores.php','principal');
	}
	else if (pagina==2)
	{
...
function actualizarTotal(idJugador, pago, abono)
{
	...
	for (x=0; x<i.length; x++){
		var nombre = i[x].getAttribute("name");
		var tagname = nombre.split("_");
		if (tagname[1] == idJugador){
			if (tagname[0] != 'Total'){
				if (numeroAbono == abono){
					if (pago.value != ''){
						total = total + parseInt(pago.value);
					}
					i[x].setAttribute("value", pago.value);
				} else {
					var valor = i[x].getAttribute("value");
					if (valor != '' && valor != null){
